...
export function Footer() {
  ...
  return <footer className="bg-background">
    <div className="max-w-7xl mx-auto px-5 sm:px-8 py-8"><div className="rounded-2xl p-7 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-7 bg-gradient-to-r from-[#1a321c] via-[#293f16] to-[#1c2730] text-white">
      <div><p className="text-sm font-bold tracking-wider text-white/60 mb-3">HETI ESEMÉNYLISTA</p><h2 className="text-2xl md:text-3xl font-extrabold">A következő programok<br /><span className="text-amber-400">egy e-mailben.</span></h2></div>
    ...
    <div className="max-w-7xl mx-auto px-5 sm:px-8 py-10 flex flex-wrap items-center justify-between gap-6">
      <div><Link href="/" className="text-2xl font-extrabold tracking-tight">hellocsík<span className="text-primary">.ro</span></Link><p className="text-sm text-muted-foreground mt-2">Programok Székelyföld városaiból.</p></div>
      <nav aria-label="Lábléc" className="flex flex-wrap gap-6 text-sm font-semibold"><a href="/#kozelgo">Programok</a><Link href="/naptar">Naptár</Link><Link href="/helyszinek">Helyszínek</Link><Link href="/erdekel">Érdekel</Link><Link href="/bekuldese" className="flex items-center gap-2 text-primary">Program beküldése <ArrowRight size={16} /></Link><Link href="/admin">Szerkesztőség</Link></nav>
    </div>
    <div className="max-w-7xl mx-auto px-5 sm:px-8 py-5 border-t border-border text-sm text-muted-foreground">© {new Date().getFullYear()} HelloCsík · Székelyföldi programok <p className="mt-2 text-xs"><a className="underline" href="https://commons.wikimedia.org/wiki/File:RO_HR_Miercurea_Ciuc_center_1.jpg" target="_blank" rel="noreferrer">Fotó: Andrei Stroe / Wikimedia Commons</a> · <a className="underline" href="https://creativecommons.org/licenses/by-sa/3.0/" target="_blank" rel="noreferrer">CC BY-SA 3.0</a> · Vágott kép</p></div>
  </footer>;
}
