        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {isLoading
            ...
            : (showAll ? events : events.slice(0, 12)).map((event, i) => {
              const isPaid = !!(event.price && event.price !== "Ingyenes");
              ...
              return (
                ...
                >
                  <Link href={`/esemeny/${event.id}`}>
                    <div className={`group cursor-pointer rounded-2xl overflow-hidden border transition-all duration-300 hover:shadow-xl h-full flex flex-col ${
                      isFeatured
                        ? "bg-gradient-to-br from-amber-50 to-card border-amber-300/70 shadow-lg shadow-amber-100/60 ring-1 ring-amber-200/60"
                        : "bg-card border-card-border hover:border-primary/20"
                    }`}>
                      {/* Image */}
                      <div className="relative overflow-hidden" style={{ aspectRatio: isFeatured ? "2/1" : "16/9" }}>
                        <img
                          src={event.imageUrl}
                          alt={event.title}
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                        />
                        <div className={`absolute inset-0 bg-gradient-to-t ${isFeatured ? "from-black/50 to-transparent" : "from-black/30 to-transparent"}`} />
                        {/* Top badges */}
                        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
                          {isFeatured && (
                            <span className="flex items-center gap-1 text-[10px] font-black px-2.5 py-1 rounded-full text-white shadow-md"
                        ...
                        {isPaid && (
                          <div className="absolute top-3 right-3 flex items-center gap-1 bg-white/95 rounded-full px-2.5 py-1 shadow-sm">
                            ...
                            <span className="text-[11px] font-bold text-primary">{event.price}</span>
                          </div>
                        )}
                        {/* Admin edit button */}
                        {isAdmin && (
                          <a
                            ...
                            className="absolute bottom-2 right-2 z-10 flex items-center gap-1 px-2 py-1 rounded-lg bg-black/70 text-white text-[10px] font-bold hover:bg-black/90 transition-colors"
                          >
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        ...
                        <h3 className={`font-bold text-foreground leading-snug mb-2 line-clamp-2 group-hover:text-primary transition-colors flex-1 ${isFeatured ? "text-lg" : "text-base"}`}>
                          {event.title}
                        </h3>
                        <div className="flex items-center justify-between mt-auto pt-2 border-t border-border/50">
                          <span className={`text-xs font-medium ${isPaid ? "text-primary" : "text-muted-foreground"}`}>
                            {isPaid ? event.price : "Ingyenes"}
                          </span>
