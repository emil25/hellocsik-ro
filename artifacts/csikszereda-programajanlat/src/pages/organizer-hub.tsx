...
export default function OrganizerHubPage() {
  ...
  const highlightRequested = new URLSearchParams(location.split("?")[1] ?? "").get("kiemeles") === "1";
  ...
  return (
    <div className="organizer-hub-page">
      ...
      <main className="organizer-hub-main">
        {highlightRequested && <div className="organizer-highlight-request"><CheckCircle2 size={18} /><span>A kiemelési csomag kiválasztva. Belépés után küldd be az eseményt, és felvesszük veled a kapcsolatot az egyszeri díjról.</span></div>}
        {organizer ? <section className="organizer-dashboard">
          ...
          <div className="organizer-dashboard-actions"><Link href={`/bekuldese?organizer=${encodeURIComponent(organizer.slug)}`} className="organizer-primary-button"><Plus size={16} /> Új esemény beküldése</Link><Link href={`/szervezo/${organizer.slug}`} className="organizer-secondary-button"><UserRound size={15} /> Profil megtekintése</Link></div>
